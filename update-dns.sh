...
if [ "$IP4ADDR" != "" ]
then
	ping $HOSTNAME_DYNV6 -4 -c 1 > /dev/null # a little dirty - needed to update dns-cache
...
if [ "$IP6ADDR" != "" ]
then
        ping $HOSTNAME_DYNV6 -6 -c 1 > /dev/null # a little dirty - needed to update dns-cache
